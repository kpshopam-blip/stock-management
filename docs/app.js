...
let allProducts = [];
let cart = [];
let cartPrices = {}; // เก็บประเภทราคาที่เลือกในตะกร้า { productId: 'retail' | 'wholesale' }
...
window.onload = function () {
    ...
    // ตั้งเวลาซิงค์อัปเดตข้อมูลสินค้าทุกๆ 3 นาที เพื่อดึงสถานะการจองสินค้าของพนักงานคนอื่น
    setInterval(() => {
...
function executeSearch(query, brand, location) {
    ...
    if (query) filtered = filtered.filter(p =>
        ...
        (p.color && String(p.color).toLowerCase().includes(query)) || 
        (p.storage && String(p.storage).toLowerCase().includes(query)) ||
        (p.imei && String(p.imei).toLowerCase().includes(query))
    );
...
async function fetchInventoryData() {
    ...
    if (tbody) tbody.innerHTML = `<tr><td colspan="10" class="px-4 py-8 text-center text-gray-500"><i class="fa-solid fa-circle-notch fa-spin mr-2"></i> กำลังโหลดข้อมูล...</td></tr>`;
...
function renderInventoryTable(products) {
    ...
    if (products.length === 0) {
        if (tbody) tbody.innerHTML = `<tr><td colspan="10" class="px-4 py-8 text-center text-gray-500">ไม่มีข้อมูลสินค้าในสต็อก</td></tr>`;
...
async function loadDashboard() {
    ...
    if (isManager) {
        // สำหรับผู้จัดการ: เปิดหน้าต่างแดชบอร์ดรายงานเต็มรูปแบบแยกต่างหาก (เปิดแท็บใหม่)
        window.open('dashboard.html', '_blank');
    } else {
        // สำหรับพนักงาน: โหลดหน้าแดชบอร์ดย่อภายในหน้าจอหลัก POS
        ...
        try {
            ...
            showToast('เกิดข้อผิดพลาดในการดึงข้อมูลแดชบอร์ด: ' + err.message, 'error');
        } finally {
            showLoading(false);
        }
    }
...
function renderSalesList(list) {
    ...
    billList.forEach(bill => {
        const profitClass = bill.totalProfit >= 0 ? 'text-green-600' : 'text-red-600';
        const isBulk = bill.items.length > 1;
        
        // สร้าง HTML สำหรับแสดงรายการสินค้า
        let itemsListHtml = '';
        if (isBulk) {
            itemsListHtml = `
            <div class="space-y-1.5 my-2 border-l-2 border-indigo-200 pl-2 bg-indigo-50/20 p-2 rounded">
                ${bill.items.map(item => `
                    <div class="text-xs text-gray-700 flex justify-between gap-2">
                        <div>
                            <span class="font-bold text-gray-800">${item.brand} ${item.model}</span>
                            <span class="text-[10px] text-gray-400 block font-mono">IMEI: ${item.imei || '-'} | ${item.spec}</span>
                        </div>
                        <span class="text-brand-600 font-semibold self-center">฿${formatNumber(item.soldPrice)}</span>
                    </div>
                `).join('')}
            </div>`;
        } else {
            const single = bill.items[0];
            itemsListHtml = `
            <div class="mb-2">
                <div class="font-bold text-gray-800 text-sm">${single.brand} ${single.model}</div>
                <div class="text-xs text-gray-500 font-mono">IMEI: ${single.imei || '-'} | ${single.spec}</div>
            </div>`;
        }
        ...
        const receiptObj = {
            ...
            costPrice: bill.items[0].cost,
            profit: bill.items[0].profit
        };
      ...
      <div class="bg-gray-50 border rounded-lg p-3 hover:bg-gray-100 transition shadow-sm">
        <div class="flex justify-between items-start mb-1 pb-1 border-b border-gray-200">
          ...
          <div class="text-right shrink-0">
            ...
            ${isManager ? `<div class="text-[10px] ${profitClass}">กำไรบิล: ฿${formatNumber(bill.totalProfit)}</div>` : ''}
          </div>
        </div>
        ...
        <div class="grid grid-cols-2 gap-1 text-xs text-gray-500 mt-1">
          <div><i class="fa-regular fa-calendar mr-1"></i>${bill.saleDate}</div>
...
async function loadSettings() {
    try {
        ...
        // ความจุ (Storage)
        const storageSel = document.getElementById('p_storage');
        if (storageSel) {
            const oldVal = storageSel.value;
            storageSel.innerHTML = '<option value="">เลือกความจุ</option>';
            if (settings.storages) {
                settings.storages.forEach(s => { storageSel.innerHTML += `<option value="${s}">${s}</option>`; });
            }
            if (oldVal) storageSel.value = oldVal;
        }

        // แหล่งที่มา (Source)
        const sourceList = document.getElementById('sourceOptions');
        if (sourceList) {
            sourceList.innerHTML = '';
            if (settings.sources) {
                settings.sources.forEach(src => { sourceList.innerHTML += `<option value="${src}">`; });
            }
        }
...
// อัปเดตรายชื่อรุ่นตามยี่ห้อที่เลือก
function updateModelDatalist(brandModels, selectedBrand) {
    ...
    if (selectedBrand && brandModels[selectedBrand]) {
        brandModels[selectedBrand].forEach(m => {
            modelList.innerHTML += `<option value="${m}">`;
        });
    } else {
...
// จับ Event change ยี่ห้อในฟอร์มรับเข้าสินค้า
function onBrandChange(selectedBrand) {
    if (window._appSettings && window._appSettings.brandModels) {
...
async function submitProduct(event) {
    ...
    const rawModel = document.getElementById('p_model').value.trim();
    const modelCode = document.getElementById('p_model_code') ? document.getElementById('p_model_code').value.trim() : '';
...
function toggleDownPayment() {
    ...
    if (isPartner) {
        ...
        if (partnerPayType === 'เชื่อ') {
            ...
            if (dueContainer) {
                ...
                if (sellDueDate && !sellDueDate.value) {
                    ...
                    const pad = (n) => n.toString().padStart(2, '0');
                    sellDueDate.value = `${d.getFullYear()}-${pad(d.getMonth() + 1)}-${pad(d.getDate())}`;
                }
            }
            if (dpContainer) dpContainer.classList.remove('hidden');
        } else {
            // ซ่อนเมื่อเลือกพาร์ทเนอร์ขายสด
            if (dueContainer) {
        ...
        if (partnerPayContainer) partnerPayContainer.classList.add('hidden');
        
        // โลจิกเดิมสำหรับประเภทอื่นๆ: แสดงช่องเงินดาวน์ทุกกรณี ยกเว้นยังไม่ได้เลือก, มีคำว่า "สด"
        if (type && !type.includes('สด')) {
...
function openSellModal(productId) {
    ...
    const dueInput = document.getElementById('sell_dueDate');
    ...
    const btnUseRetail = document.getElementById('btnUseRetailPrice');
    const btnUseWholesale = document.getElementById('btnUseWholesalePrice');
    if (btnUseRetail) {
        btnUseRetail.onclick = () => {
...
async function confirmSell() {
    ...
    // ตรวจสอบข้อมูลพาร์ทเนอร์และเงื่อนไขการขายเชื่อตามข้อกำหนด
    const isPartner = saleType && saleType.includes('พาร์ทเนอร์');
    const partnerPayType = isPartner ? (document.querySelector('input[name="partner_payment_type"]:checked')?.value || 'สด') : '';
    const isPartnerCredit = isPartner && partnerPayType === 'เชื่อ';
    ...
    if (isPartner) {
        finalSaleType = `${saleType} (เงิน${partnerPayType})`;
    }
...
function showReceipt(r) {
    ...
    if (r.isBulk && Array.isArray(r.items)) {
        r.items.forEach(item => {
            ...
            itemsHtml += `
            <div class="bg-gray-50 p-2.5 rounded mb-1.5 border border-gray-100">
                <div class="font-bold text-gray-800 text-sm">${item.brand} ${item.model}</div>
                <div class="text-[11px] text-gray-500">สเปค: ${item.spec} | IMEI: ${item.imei || '-'}</div>
                <div class="text-right text-brand-600 font-bold text-xs mt-0.5">฿${formatNumber(item.soldPrice)}</div>
            </div>`;
        });
    } else {
        ...
        <div class="bg-gray-50 p-2 rounded border border-gray-100">
            <div class="font-bold text-gray-800 text-sm">${r.brand} ${r.model}</div>
            <div class="text-[11px] text-gray-500">สเปค: ${r.spec} | IMEI: ${r.imei || '-'}</div>
        </div>`;
    }
    ...
    document.getElementById('receiptBody').innerHTML = `
        ...
        <div class="space-y-2 text-xs">
            ...
            <div class="flex justify-between text-sm"><span class="text-gray-500">ยอดจัด:</span><span class="font-medium text-red-500">฿${formatNumber(totalAmount - r.downPayment)}</span></div>
            ` : ''}
            <hr>
...
// ====== ระบบอ่าน IMEI 1 / Serial Number จากรูปภาพ (Image OCR & Barcode) ======
let activeOcrTargetInputId = 'p_imei';
let currentOcrImageCompressedUri = null;
let currentOcrFileName = '';
let currentOcrResult = null;

function triggerOcrFileInput(targetInputId = 'p_imei') {
    activeOcrTargetInputId = targetInputId || 'p_imei';
    const fileInput = document.getElementById('ocr_global_file_input');
    if (fileInput) {
        fileInput.value = '';
        fileInput.click();
    }
...
function closeOcrModal() {
    ...
    const beam = document.getElementById('ocrScanBeam');
    if (beam) beam.classList.add('hidden');
}

// ปรับแต่งภาพเบื้องต้น (Contrast & Grayscale) เพื่อเพิ่มความแม่นยำในการอ่าน OCR
function preprocessImageForOcr(img) {
    const canvas = document.createElement('canvas');
    let width = img.width;
    let height = img.height;
    
    // ควบคุมขนาดสูงสุดเพื่อความเร็ว ไม่เกิน 1800px
    const MAX_DIM = 1800;
    ...
    // ดึง ImageData เพื่อปรับ contrast และ grayscale เล็กน้อย
    try {
        const imgData = ctx.getImageData(0, 0, width, height);
        const d = imgData.data;
        const contrast = 1.25; // เพิ่ม contrast 25%
...
// แยกแยะและดึงเฉพาะ IMEI 1 หรือ Serial Number ตามเงื่อนไขอัจฉริยะ
function extractImeiOrSerial(rawText, barcodes = []) {
    ...
    // 1. ตรวจสอบจากบาร์โค้ดก่อน (หากเครื่องอ่าน native BarcodeDetector สแกนบาร์โค้ดได้)
    ...
    // 2. ตรวจสอบข้อความจาก OCR:
    // 2.1 มีคำว่า IMEI1 หรือ IMEI 1 อย่างชัดเจน (เช่น IMEI1:864738045816451 หรือ IMEI 1 : 8647...)
    const imei1Explicit = text.match(/IMEI\s*1\s*[:\s-]*([0-9]{15})/i);
    if (imei1Explicit) {
        return { type: 'IMEI 1', value: imei1Explicit[1] };
    }
    
    // 2.2 มีคำว่า IMEI และ IMEI 2 ในภาพ (เช่น บน iPhone หรือหน้าจอทั่วไป)
    // เราจะตัดข้อความตั้งแต่คำว่า IMEI 2 ออกไป เพื่อให้ค้นหาเฉพาะส่วนของ IMEI 1 เท่านั้น!
    const textParts = text.split(/IMEI\s*2/i);
    const beforeImei2 = textParts[0];
    
    const imeiLeadingMatch = beforeImei2.match(/IMEI\s*[:\s-]*([0-9]{15})/i);
    if (imeiLeadingMatch) {
        return { type: 'IMEI 1', value: imeiLeadingMatch[1] };
    }
    
    // 2.3 ตรวจหาตัวเลข 15 หลักชุดแรกที่ปรากฏในส่วนก่อน IMEI 2 (กรณีคำว่า IMEI ตรวจจับไม่สมบูรณ์)
    const fifteenDigitsBeforeImei2 = beforeImei2.match(/\b([0-9]{15})\b/);
    if (fifteenDigitsBeforeImei2) {
        return { type: 'IMEI 1', value: fifteenDigitsBeforeImei2[1] };
    }
    
    // 2.4 ตรวจหาตัวเลข 15 หลักใดๆ ในข้อความทั้งหมด (กรณีเครื่องมี IMEI เดียว)
    const anyFifteen = text.match(/\b([0-9]{15})\b/);
    if (anyFifteen) {
        return { type: 'IMEI 1', value: anyFifteen[1] };
    }
    
    // 3. หากไม่พบ IMEI (เช่น iPad Wi-Fi ที่ไม่มีซิม) -> ตรวจหา Serial Number (SN)
    // คำบอกใบ้: Serial No, Serial Number, S/N, SN, หมายเลขประจำเครื่อง (ความยาว 8 - 14 ตัวอักษร)
    ...
    // 4. บาร์โค้ดที่มีความยาวระหว่าง 8-14 ตัวอักษร (ไม่ใช่ EID 32 หลัก)
    if (barcodes && barcodes.length > 0) {
...
// ฟังก์ชันหลักในการเปิด Modal และประมวลผลรูปภาพ
async function handleOcrImageFile(input) {
    if (!input.files || input.files.length === 0) return;
    const file = input.files[0];
    ...
    const scanBeam = document.getElementById('ocrScanBeam');
    ...
    statusText.innerText = 'กำลังโหลดรูปภาพ...';
    
    currentOcrImageCompressedUri = null;
    ...
    reader.onload = async function(e) {
        ...
        img.onload = async function() {
            try {
                // ขั้นตอนที่ 1: Pre-processing ภาพบน Canvas เพื่อให้คมชัดและขนาดพอดี
                statusText.innerText = 'กำลังปรับความคมชัดของภาพ...';
                const processedCanvas = preprocessImageForOcr(img);
                
                // เก็บภาพที่ย่อแล้วในรูปแบบ WebP เผื่อแนบเข้าสินค้า
                try {
                    currentOcrImageCompressedUri = processedCanvas.toDataURL('image/webp', 0.8);
                } catch (err) {
                    currentOcrImageCompressedUri = processedCanvas.toDataURL('image/jpeg', 0.8);
                }
                
                // ขั้นตอนที่ 2: ตรวจหาบาร์โค้ดในภาพ (Barcode Detection)
                let detectedBarcodes = [];
                if ('BarcodeDetector' in window) {
                    try {
                        statusText.innerText = 'กำลังตรวจจับแถบบาร์โค้ด...';
                        const barcodeDetector = new BarcodeDetector({ 
                            formats: ['code_128', 'ean_13', 'upc_a', 'code_39', 'qr_code'] 
                        });
                        const barcodes = await barcodeDetector.detect(processedCanvas);
                        if (barcodes && barcodes.length > 0) {
                            detectedBarcodes = barcodes.map(b => b.rawValue);
                        }
                    } catch (bcErr) {
                        console.warn('BarcodeDetector error:', bcErr);
                    }
                }
                
                // ขั้นตอนที่ 3: ดำเนินการ OCR อ่านตัวหนังสือด้วย Tesseract.js
                statusText.innerText = 'กำลังอ่านตัวเลขและข้อความ OCR...';
                let ocrText = '';
                
                if (typeof Tesseract !== 'undefined') {
                    const ocrResponse = await Tesseract.recognize(
                        processedCanvas,
                        'eng',
                        {
                            logger: m => {
                                if (m.status === 'recognizing text' && m.progress) {
                                    statusText.innerText = `กำลังอ่านตัวอักษร (${Math.round(m.progress * 100)}%)...`;
                                }
                            }
                        }
                    );
                    ocrText = (ocrResponse && ocrResponse.data) ? ocrResponse.data.text : '';
                } else {
                    console.warn('Tesseract.js not loaded!');
                }
                
                // ขั้นตอนที่ 4: สกัดหา IMEI 1 หรือ Serial Number
                statusText.innerText = 'กำลังวิเคราะห์ผลลัพธ์...';
                const result = extractImeiOrSerial(ocrText, detectedBarcodes);
                ...
                if (result && result.value) {
                    ...
                    document.getElementById('ocrResultType').innerText = result.type;
                    document.getElementById('ocrExtractedValue').value = result.value;
                    resultContainer.classList.remove('hidden');
                    btnConfirm.disabled = false;
                    
                    showToast(`อ่านสำเร็จ: ${result.value} (${result.type})`, 'success');
                } else {
                    errorContainer.classList.remove('hidden');
                    showToast('ไม่พบเลข IMEI หรือ Serial Number ในรูปนี้', 'warning');
                }
                
            } catch (procErr) {
                console.error('OCR Processing error:', procErr);
                loadingOverlay.classList.add('hidden');
                scanBeam.classList.add('hidden');
                errorContainer.classList.remove('hidden');
                showToast('เกิดข้อผิดพลาดในการประมวลผลรูปภาพ: ' + procErr.message, 'error');
            }
        };
        img.src = e.target.result;
    };
...
// กดยืนยันนำค่าไปใส่ในช่อง IMEI
function confirmOcrResult() {
    const extractedVal = document.getElementById('ocrExtractedValue').value.trim();
    if (!extractedVal) {
        showToast('กรุณาระบุตัวเลขหรือรหัสก่อนกดยืนยัน', 'warning');
        return;
    }
    
    const targetInput = document.getElementById(activeOcrTargetInputId || 'p_imei');
    if (targetInput) {
        targetInput.value = extractedVal;
        
        // หากเป้าหมายคือช่อง p_imei ให้เรียกฟังก์ชันตรวจสอบ IMEI ซ้ำทันที
        if (activeOcrTargetInputId === 'p_imei') {
            onImeiInput(extractedVal);
        } else if (activeOcrTargetInputId === 'inventorySearch') {
            filterInventory();
        } else if (activeOcrTargetInputId === 'storeSearchDesktop' || activeOcrTargetInputId === 'storeSearchMobile') {
            searchProducts(activeOcrTargetInputId);
        } else if (activeOcrTargetInputId === 'cartImeiInput') {
            addToCartByImei(extractedVal);
        }
    }
...
async function clearCart() {
    ...
    Promise.all(itemsToUnlock.map(id => 
        API_unlockProduct(id).then(res => {
            ...
            console.error(`Unlock network error for ${id}:`, e);
            return false;
        })
    )).then(results => {
        showLoading(false); // ปิดตัวหมุน
...
function checkoutCart() {
    ...
    checkoutProductIds.forEach(pId => {
        ...
        <div class="bg-gray-50 p-2.5 rounded border border-gray-200 flex flex-col gap-1">
          ...
          <div class="flex items-center gap-2 mt-1">
            ...
            <input type="number" id="bulk_price_${p.id}" value="${defaultPrice}" class="w-full text-xs font-bold text-brand-600 p-1 border border-gray-300 rounded focus:ring-brand-500 outline-none bg-brand-50">
    ...
    // เติมประเภทรูปแบบการขายใน Dropdown
    const typeSel = document.getElementById('sell_type');
    typeSel.innerHTML = '<option value="">เลือกรูปแบบ</option>';
    if (window._appSettings && window._appSettings.saleTypes) {
        window._appSettings.saleTypes.forEach(t => { typeSel.innerHTML += `<option value="${t}">${t}</option>`; });
    } else {
        API_getSettings().then(s => { 
            window._appSettings = s; 
            s.saleTypes.forEach(t => typeSel.innerHTML += `<option value="${t}">${t}</option>`); 
        }).catch(() => { });
    }

    // แสดง Modal สำหรับกรอกข้อมูลขาย
    document.getElementById('sellModal').classList.remove('hidden');
    toggleDownPayment();
}

function setBulkPrice(productId, price) {
    const input = document.getElementById(`bulk_price_${productId}`);
    if (input) {
        input.value = price;
    }
}
